...
function create() {
  ...
  level.greenTurtles.forEach((greenTurtle) => {
    ...
    // Green Turtle & Player collision - handled inside Green Turtle class
    this.physics.add.collider(player.sprite, greenTurtle.sprite, function (playerSprite, greenTurtleSprite) {
...
// Update function to include Green Turtle updates
function update(time) {
  ...
  level.greenTurtles.forEach((greenTurtle) => greenTurtle.update(time));
...
class GreenTurtle {
  ...
  update(time) {
    ...
    if (time > this.jumpTimer) {
      ...
      this.jumpTimer = time + Phaser.Math.Between(2000, 5000);
    }

    // Throw hammer occasionally
    if (time > this.throwHammerTimer) {
      this.throwHammer();
      this.throwHammerTimer = time + Phaser.Math.Between(3000, 6000);
    }
  }

  throwHammer() {
    const hammer = this.scene.physics.add.sprite(this.sprite.x, this.sprite.y, 'hammer');
    hammer.setVelocityX(this.direction * 200);
    hammer.body.allowGravity = false;

    this.scene.physics.add.collider(hammer, this.scene.level.platforms.group, () => {
      hammer.destroy();
    });

    this.scene.physics.add.overlap(this.scene.player.sprite, hammer, () => {
      hammer.destroy();
      this.scene.endGame();
    });
  }
  ...
  handlePlayerCollision(playerSprite, playerObj, onStompedCallback, onGameOverCallback) {
    if (this.isShell) {
      // If Mario hits the shell from the side
      if (playerSprite.body.touching.left || playerSprite.body.touching.right) {
        if (onGameOverCallback) onGameOverCallback();
      }
    } else {
